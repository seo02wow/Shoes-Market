 		...
 		<h4 id="board_title">문의게시글 리스트</h4>
			...
			<table class="table table-hover">
					...
					<c:otherwise>
						<c:forEach var="vo" items="${list }">
							<tr>
								...
								<td>
								<a href="controller?type=managerInquiryView&inquiryNum=${vo.inquiryNum}&idx=${idx}&cPage=${cPage}&keyword=${keyword}">${vo.title }</a>
								</td>
								<td>${vo.itemNum }</td>
								<td>${vo.regDate }</td>
								<td>${vo.modDate }</td>
				...
				<td colspan="6">
					...
					<c:if test="${pvo.beginPage != 1 }">
						<li>
							<a href="controller?type=managerInquiry&cPage=${pvo.beginPage - 1 }">
							<i class="fa fa-angle-left" style="font-size:24px"></i>
							</a>
						</li>
					</c:if>	
					...
					<c:if test="${pageNo != pvo.nowPage }">
						<li>
							<a href="controller?type=managerInquiry&cPage=${pageNo }">${pageNo }</a>
						</li>
					</c:if>		
					...
					<c:if test="${pvo.endPage < pvo.totalPage }">
						<li>
							<a href="controller?type=managerInquiry&cPage=${pvo.endPage + 1 }">
